...
export class MemStorage implements IStorage {
    ...
    async createEnvLog(insertLog: InsertEnvLog): Promise<EnvLog> {
        ...
        let alert = false;
        if (room) {
            alert = insertLog.temperature > room.tempThreshold || insertLog.humidity > room.humidityThreshold;
    ...
    async getUnreadMessageCountForUser(userId: number): Promise<number> {
        return Array.from(this.messages.values()).filter((msg) => msg.receiverId === userId && !msg.read).length;
    }
}
...
export class DatabaseStorage implements IStorage {
    ...
    async updatePatient(id: number, patientData: Partial<Patient>): Promise<Patient | undefined> {
        const [updatedPatient] = await db.update(patients).set(patientData).where(eq(patients.id, id)).returning();
        return updatedPatient;
    }
    ...
    async getPatientWithDetails(id: number): Promise<PatientWithDetails | undefined> {
        ...
        const [user] = patient.userId ? await db.select().from(users).where(eq(users.id, patient.userId)) : [undefined];

        const [room] = patient.roomId ? await db.select().from(rooms).where(eq(rooms.id, patient.roomId)) : [undefined];
    ...
    async updateGuardian(id: number, guardianData: Partial<Guardian>): Promise<Guardian | undefined> {
        const [updatedGuardian] = await db.update(guardians).set(guardianData).where(eq(guardians.id, id)).returning();
        return updatedGuardian;
    }
    ...
    async updateAccident(id: number, accidentData: Partial<Accident>): Promise<Accident | undefined> {
        const [updatedAccident] = await db.update(accidents).set(accidentData).where(eq(accidents.id, id)).returning();
        return updatedAccident;
    }
    ...
    async getAccidentsByRoomId(roomId: number): Promise<Accident[]> {
        return await db.select().from(accidents).where(eq(accidents.roomId, roomId)).orderBy(desc(accidents.date));
    }

    async getRecentAccidents(limit: number): Promise<Accident[]> {
        return await db.select().from(accidents).orderBy(desc(accidents.date)).limit(limit);
    }
    ...
    async getEnvLogsByRoomId(roomId: number, limit?: number): Promise<EnvLog[]> {
        const query = db.select().from(envLogs).where(eq(envLogs.roomId, roomId)).orderBy(desc(envLogs.timestamp));
    ...
    async updateCamera(id: number, cameraData: Partial<Camera>): Promise<Camera | undefined> {
        const [updatedCamera] = await db.update(cameras).set(cameraData).where(eq(cameras.id, id)).returning();
        return updatedCamera;
    }
    ...
    async updateMessage(id: number, messageData: Partial<Message>): Promise<Message | undefined> {
        const [updatedMessage] = await db.update(messages).set(messageData).where(eq(messages.id, id)).returning();
        return updatedMessage;
    }
...
async function initializeData() {
    ...
    const adminCheck = await db.select().from(users).where(eq(users.role, UserRole.DIRECTOR)).limit(1);
